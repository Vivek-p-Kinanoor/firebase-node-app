...
const checkArticleSpellingFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    try {
      ...
      console.error("Error in checkArticleSpellingFlow:", e);
      // Check for common API error messages to provide a better user-facing error.
      if (e.message?.includes('Service Unavailable') || e.message?.includes('503')) {
          throw new Error("The AI spell check service is temporarily unavailable. Please try again in a few moments.");
      }
      throw new Error("An unexpected error occurred while checking spelling. Please try again.");
    }
  }
);
